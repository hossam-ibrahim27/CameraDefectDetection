...
function DefectDetector(): React.JSX.Element {
  ...
  const [imgScale, setImgScale] = useState<{ scaleX: number; scaleY: number }>({
    scaleX: 1,
    scaleY: 1
  })
  ...
  //@ *********************** Demo **********************************
  return (
    <main className="h-screen w-screen bg-stone-50 text-stone-900 flex flex-col md:flex-row font-sans overflow-hidden">
      <Toaster
        position="top-center"
        toastOptions={{
          style: {
      ...
      />
      <nav className="w-full md:w-64 p-4 md:p-6 flex flex-row md:flex-col items-center md:items-stretch border-b md:border-r border-stone-200 bg-white z-10 shrink-0">
        <header className="mb-0 md:mb-10 text-left md:text-center shrink-0">
          ...
          <p className="text-stone-500 text-xs mt-1 hidden md:block">
            {' '}
            Camera Defect Detection System{' '}
          </p>
        </header>
          ...
          {navItems.map((item, index) => (
            <li key={index}>
              <button
                ...
                className={`w-auto md:w-full flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-3 rounded-full md:rounded-lg text-xs md:text-sm font-medium transition ${showLiveStream === item.isLive ? 'bg-olive-50 text-olive-950 shadow-inner' : 'text-stone-500 hover:bg-stone-100/50 hover:text-stone-900'}`}
              >
                {item.label}
              </button>
            </li>
        ...
        <footer className="mt-auto text-center border-t border-stone-200 pt-6 hidden md:block">
          ...
          <p className="text-stone-600 text-xs mt-1">
            &copy; 2026 Al-Azher Team. All Rights Reserved.
          </p>
        </footer>
      </nav>
      {/* --- Main Content Area --- */}
      <section className="flex-1 flex flex-col min-w-0">
        <header className="h-auto md:h-16 border-b border-stone-200 bg-white p-4 md:px-6 flex flex-col md:flex-row items-center justify-center text-sm gap-3 md:gap-6">
          <div className="flex items-center justify-center md:justify-center gap-3 md:gap-5 text-xs text-stone-500 border-t md:border-t-0 border-stone-100 pt-3 md:pt-0">
            <span
              className={`flex items-center gap-1 md:gap-1.5 ${streamError ? 'text-red-600' : 'text-emerald-700'}`}
            >
              <span
                className={`h-1.5 md:h-2 w-1.5 md:w-2 rounded-full ${streamError ? 'bg-red-600' : 'bg-emerald-600'}`}
              ></span>
                  ...
                  onError={(e) => {
                    ...
                    setTimeout(() => {
                      ;(e.target as HTMLImageElement).src =
                        `${server_api}/video_feed?t=${new Date().getTime()}`
                    }, 3000)
                  }}
                ...
                {streamError && (
                  <div className="absolute inset-0 flex flex-col justify-center items-center gap-3 md:gap-4 text-center p-4 md:p-6 bg-stone-100/80 backdrop-blur-sm z-20">
                    ...
                    <p className="text-xs md:text-sm text-stone-600 max-w-md">
                      Could not connect to the live video stream. Please check server and camera
                      settings.
                    </p>
                    <button
                      onClick={() => {
              ...
              <div className="flex-1 flex flex-col space-y-4 md:space-y-6 min-h-0">
                      ...
                      {results &&
                        ...
                        results.detections.map((detection: Detection, index: number) => {
                          ...
                          return (
                            <div
                              key={index}
                              className={`absolute border rounded ${getClassColor(detection.class)} transition-all pointer-events-none`}
                              style={{
                                left: `${left}px`,
                                top: `${top}px`,
                                width: `${width}px`,
                    ...
                    {results && (
                      <div className="absolute bottom-2 md:bottom-4 left-2 md:left-4 bg-emerald-50 text-emerald-900 px-2.5 py-1 md:px-4 md:py-1.5 rounded-full border border-emerald-200 shadow-lg text-[10px] md:text-sm font-semibold z-10">
                        ...
                        {results.total_defects > 0
                          ? `${results.total_defects} Defect(s) Detected`
                          : 'OK - No Defects'}
                      </div>
                    )}
                  </div>
                ...
                {!previewUrl && (
                  <div className="flex-1 flex flex-col justify-center items-center gap-3 md:gap-4 text-center p-6 md:p-10 bg-stone-100 rounded-xl border border-stone-200 border-dashed min-h-0">
                    ...
                    <p className="text-xs md:text-sm text-stone-600 max-w-xs">
                      Select a test image from your device to begin the analysis and detection
                      process.
                    </p>
                  </div>
                )}
